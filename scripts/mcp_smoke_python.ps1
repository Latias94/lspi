param(
  # Workspace root (project root). If omitted, defaults to samples/python/Hello.
  ...
  # Optional: override the language server command/args (default: pyright-langserver --stdio).
  [Parameter(Mandatory = $false)]
  [string]$LspCommand = "pyright-langserver",
...
function Resolve-WorkspaceRoot($workspaceRoot) {
  ...
  return (Resolve-Path -LiteralPath "samples/python/Hello").Path
}

function Pick-PyFile($projectRoot) {
  $preferred = Join-Path -Path $projectRoot -ChildPath "src\\main.py"
  if (Test-Path -LiteralPath $preferred -PathType Leaf) { return $preferred }
  $files = Get-ChildItem -LiteralPath $projectRoot -Recurse -Filter *.py -File -ErrorAction SilentlyContinue |
...
if (-not $lspOk -and $LspCommand -eq "pyright-langserver") {
  $localCmd = Join-Path $resolvedWorkspaceRoot "node_modules\\.bin\\pyright-langserver.cmd"
  if (Test-Path -LiteralPath $localCmd -PathType Leaf) {
    $LspCommand = $localCmd
    $lspOk = $true
  } else {
    $localCmd = Join-Path $resolvedWorkspaceRoot "node_modules\\.bin\\pyright-langserver"
    if (Test-Path -LiteralPath $localCmd -PathType Leaf) {
      $LspCommand = $localCmd
      $lspOk = $true
    }
...
if (-not $lspOk) {
  Should-Skip "pyright-langserver is not available (run npm install in the project, install it globally, or pass -LspCommand / -LspArgs)"
}
...
if (-not [string]::IsNullOrWhiteSpace($TestFile)) {
  ...
  $testFileFull = Pick-PyFile $resolvedWorkspaceRoot
  if ($null -eq $testFileFull) {
    if ($Strict) { throw "no .py files found under: $resolvedWorkspaceRoot" }
    Should-Skip "no .py files found under: $resolvedWorkspaceRoot"
  }
}

$tmpConfig = Join-Path -Path $env:TEMP -ChildPath ("lspi-python-config-" + [Guid]::NewGuid().ToString("N") + ".toml")
...
try {
  ...
  Write-JsonLine $stdin @{
    ...
    params = @{
      ...
      capabilities = @{}
      clientInfo = @{ name = "lspi-smoke-python"; version = "0.0.0" }
    }
  }
  ...
  Write-Host "tools/call restart_server (extensions=['py']) ..." -ForegroundColor Cyan
  Write-JsonLine $stdin @{
    jsonrpc = "2.0"
    id = 9
    method = "tools/call"
