param(
  # Workspace root (project root). If omitted, defaults to samples/lua/Hello.
  ...
  # Optional: override the language server command/args.
  [Parameter(Mandatory = $false)]
  [string]$LspCommand = "lua-language-server",
...
function Resolve-WorkspaceRoot($workspaceRoot) {
  ...
  return (Resolve-Path -LiteralPath "samples/lua/Hello").Path
}

function Pick-LuaFile($projectRoot) {
  $preferred = Join-Path -Path $projectRoot -ChildPath "main.lua"
  if (Test-Path -LiteralPath $preferred -PathType Leaf) { return $preferred }
  $files = Get-ChildItem -LiteralPath $projectRoot -Recurse -Filter *.lua -File -ErrorAction SilentlyContinue |
...
if ($null -eq (Get-Command $LspCommand -ErrorAction SilentlyContinue) -and -not (Test-Path -LiteralPath $LspCommand -PathType Leaf)) {
  Should-Skip "lua-language-server is not available (install it, or pass -LspCommand / -LspArgs)"
}
...
if (-not [string]::IsNullOrWhiteSpace($TestFile)) {
  ...
  $testFileFull = Pick-LuaFile $resolvedWorkspaceRoot
  if ($null -eq $testFileFull) {
    if ($Strict) { throw "no .lua files found under: $resolvedWorkspaceRoot" }
    Should-Skip "no .lua files found under: $resolvedWorkspaceRoot"
  }
}

$tmpConfig = Join-Path -Path $env:TEMP -ChildPath ("lspi-lua-config-" + [Guid]::NewGuid().ToString("N") + ".toml")
...
try {
  ...
  Write-JsonLine $stdin @{
    ...
    params = @{
      ...
      capabilities = @{}
      clientInfo = @{ name = "lspi-smoke-lua"; version = "0.0.0" }
    }
  }
  ...
  Write-Host "tools/call restart_server (extensions=['lua']) ..." -ForegroundColor Cyan
  Write-JsonLine $stdin @{
    jsonrpc = "2.0"
    id = 9
    method = "tools/call"
