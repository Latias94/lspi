param(
  # Workspace root (project root). If omitted, defaults to samples/cpp/Hello.
  ...
  # Optional: override the language server command/args (default: clangd --stdio).
  [Parameter(Mandatory = $false)]
  [string]$LspCommand = "clangd",
...
function Resolve-WorkspaceRoot($workspaceRoot) {
  ...
  return (Resolve-Path -LiteralPath "samples/cpp/Hello").Path
}

function Pick-CppFile($projectRoot) {
  $preferred = Join-Path -Path $projectRoot -ChildPath "src\\main.cpp"
  if (Test-Path -LiteralPath $preferred -PathType Leaf) { return $preferred }
  $files = Get-ChildItem -LiteralPath $projectRoot -Recurse -Include *.cpp, *.cc, *.cxx -File -ErrorAction SilentlyContinue |
...
if ($null -eq (Get-Command $LspCommand -ErrorAction SilentlyContinue) -and -not (Test-Path -LiteralPath $LspCommand -PathType Leaf)) {
  Should-Skip "clangd is not available (install LLVM/clangd, or pass -LspCommand / -LspArgs)"
}
...
if (-not [string]::IsNullOrWhiteSpace($TestFile)) {
  ...
  $testFileFull = Pick-CppFile $resolvedWorkspaceRoot
  if ($null -eq $testFileFull) {
    if ($Strict) { throw "no .cpp files found under: $resolvedWorkspaceRoot" }
    Should-Skip "no .cpp files found under: $resolvedWorkspaceRoot"
  }
}

$tmpConfig = Join-Path -Path $env:TEMP -ChildPath ("lspi-cpp-config-" + [Guid]::NewGuid().ToString("N") + ".toml")
...
try {
  ...
  Write-JsonLine $stdin @{
    ...
    params = @{
      ...
      capabilities = @{}
      clientInfo = @{ name = "lspi-smoke-cpp"; version = "0.0.0" }
    }
  }
  ...
  Write-Host "tools/call restart_server (extensions=['cpp']) ..." -ForegroundColor Cyan
  Write-JsonLine $stdin @{
    jsonrpc = "2.0"
    id = 9
    method = "tools/call"
