param(
  # Workspace root (project root). If omitted, defaults to samples/go/Hello.
  ...
  [Parameter(Mandatory = $false)]
  [string]$Needle = "mathx.Add(",

  # If set, install gopls if missing: go install golang.org/x/tools/gopls@latest
  [Parameter(Mandatory = $false)]
  [switch]$InstallGopls = $false,

  # Optional: override the language server command/args (default: gopls serve).
  [Parameter(Mandatory = $false)]
  [string]$LspCommand = "gopls",

  [Parameter(Mandatory = $false)]
  [string[]]$LspArgs = @("serve"),
...
function Resolve-WorkspaceRoot($workspaceRoot) {
  ...
  return (Resolve-Path -LiteralPath "samples/go/Hello").Path
}

function Pick-GoFile($projectRoot) {
  $preferred = Join-Path -Path $projectRoot -ChildPath "main.go"
  if (Test-Path -LiteralPath $preferred -PathType Leaf) { return $preferred }
  $files = Get-ChildItem -LiteralPath $projectRoot -Recurse -Filter *.go -File -ErrorAction SilentlyContinue |
...
if (-not (Get-Command go -ErrorAction SilentlyContinue)) {
  Should-Skip "go is not available on PATH"
}

if ($null -eq (Get-Command $LspCommand -ErrorAction SilentlyContinue) -and -not (Test-Path -LiteralPath $LspCommand -PathType Leaf)) {
  if ($InstallGopls -and $LspCommand -eq "gopls") {
    Write-Host "Installing gopls (go install golang.org/x/tools/gopls@latest)..." -ForegroundColor Cyan
    & go install golang.org/x/tools/gopls@latest | Out-Host
  }
}

if ($null -eq (Get-Command $LspCommand -ErrorAction SilentlyContinue) -and -not (Test-Path -LiteralPath $LspCommand -PathType Leaf)) {
  Should-Skip "gopls is not available (install it, or pass -LspCommand / -LspArgs; optionally use -InstallGopls)"
}
...
if (-not [string]::IsNullOrWhiteSpace($TestFile)) {
  ...
  $testFileFull = Pick-GoFile $resolvedWorkspaceRoot
  if ($null -eq $testFileFull) {
    if ($Strict) { throw "no .go files found under: $resolvedWorkspaceRoot" }
    Should-Skip "no .go files found under: $resolvedWorkspaceRoot"
  }
}

$tmpConfig = Join-Path -Path $env:TEMP -ChildPath ("lspi-go-config-" + [Guid]::NewGuid().ToString("N") + ".toml")
...
try {
  ...
  Write-JsonLine $stdin @{
    ...
    params = @{
      ...
      capabilities = @{}
      clientInfo = @{ name = "lspi-smoke-go"; version = "0.0.0" }
    }
  }
  ...
  Write-Host "tools/call restart_server (extensions=['go']) ..." -ForegroundColor Cyan
  Write-JsonLine $stdin @{
    jsonrpc = "2.0"
    id = 9
    method = "tools/call"
