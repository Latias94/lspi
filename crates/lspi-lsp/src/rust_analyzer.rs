...
use crate::lsp::{
    LspClient, LspClientOptions, LspDiagnostic, LspDocumentSymbol, LspLocation, LspLocationLink,
    LspPosition, LspRange, LspSymbolInformation, LspTextEdit, ServerStatus,
    normalize_workspace_edit, uri_to_path,
};
    ...
    async fn definition_values_with_retry(
        ...
        for attempt in 0..3 {
            match self.lsp.definition(file_path, position.clone()).await {
                Ok(value) => match parse_definitions(value) {
                    Ok(defs) => {
                        if !defs.is_empty() {
                            return Ok(defs);
                        }
...
fn parse_symbols(value: Value) -> Result<Vec<FlatSymbol>> {
    if value.is_null() {
        return Ok(Vec::new());
    }
...
fn parse_definitions(value: Value) -> Result<Vec<Value>> {
    if value.is_null() {
        return Ok(Vec::new());
    }
    ...
    Err(anyhow!("definition response is neither array nor object"))
}

fn to_lsp_location(value: &Value) -> Result<LspLocation> {
...
fn to_resolved_location(loc: &LspLocation) -> Result<ResolvedLocation> {
